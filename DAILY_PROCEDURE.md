...
and possible fix, if the first attempt aborted). Keeping one copy here
instead of two independent trigger prompts is deliberate — the two drifting
out of sync is exactly the kind of bug this project has hit before.
...
5. Record this entire step's output — the verdict, `n`, the mean excess, the sample still needed, and the policy decision — on the run log in full; it lives in the manifest/journal either way. In the email itself (Stage 6, "System health"), compress it to one or two plain sentences: the verdict and, when `n == 0`, "no closed trades yet, roughly N more needed" — that is itself the finding, and the one that matters most on a day there is nothing else to report.
...
**Silence must never be the final outcome for a trading day.** The 06:20 routine may stay quiet on an abort (see above), but by the time the watchdog's pass is done — whether it fixed something, couldn't find a confident fix, or its own retry also aborted — exactly one email always goes out. Never fabricate a number: every figure carries a timestamp and a named source, and an unavailable figure is reported as unavailable, never estimated.
