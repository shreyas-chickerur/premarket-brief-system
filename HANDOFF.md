...
#### Status snapshot (updated after each material change; latest 5 September 2026)

| # | Item | Status |
|---|---|---|
| 1 | Self-heal proves itself, including a real fix-and-retry cycle | **Not yet.** One near-miss caught and fixed (2 September — a too-tight watchdog offset), but that was found in a dev session, not by the watchdog completing a real diagnose-fix-merge-retry against a genuinely `aborted` run. 3 September validated the *healthy* path only: the widened 60-minute offset gave the watchdog a clean first-listing read, no recheck or retry needed. Still open, unchanged by anything below — if anything there is more surface area behind it now (five new pieces on the critical path, see item 2), not less. |
| 2 | Call quality over a stretch of days | **Reset to zero, 5 September 2026.** The three sessions this item previously credited (1, 2, 3 September) ran on code substantially different from what runs Tuesday: a different Stage 1 research path, a different email renderer (`render_email` could be called without `verify_email` until the seam closed), a gate input (`sector_exposure`) that did not exist and so was silently skipped rather than checked, and sixteen commits of changes in between, three of which (`washsale_registry_stable`, the `NEWS_SENTIMENT` filter, the preview-envelope branches) can now abort or fail a run outright where nothing could before. An observation window measures the system that produced it; crediting it to a system that no longer exists overstates what has actually been watched working. The count restarts at the next real run against the current code. |
...
Net: item 5 is still genuinely done — nothing since has touched it. Item 2 is reset to zero and explicitly does not carry forward the three pre-5-September sessions as evidence about the current system; the count needs to be rebuilt against the code that will actually run Tuesday. Items 1, 4 (in its reframed form), and 6 are still open, item 1 explicitly with more surface area behind it than before. **Next scheduled reassessment: 3 weeks out (around 25 September 2026), after the OXY thesis has settled** — unchanged, since that date tracks the thesis, not the code.
