...
| `emailer.py` | HTML brief rendering, failure diagnosis (`diagnose()` now tells a lapsed brokerage token apart from a routine missing its connectors entirely — identical `tools_available` symptom, different remedy), subject lines, the five-section cap (`CANONICAL_SECTIONS`/`ACCOUNT_SECTIONS`/`OTHER_SECTIONS`) enforced in code, closest-call reporting in `idea_cards` for a day nothing clears the gate, `verify_email` — raises on an unsupported number, source, empty-source bullet, or (10 September 2026) a `buy`/`sell`/`trim` card with no parseable quantity (`ACTIONS_REQUIRING_QUANTITY`) — "partial trim" is not an answer to "how much"; `render_email` takes structured account ideas (not pre-rendered HTML) and runs `verify_email` on them itself, unconditionally, so there is no path to a sent email that skips it. `_health_line` (8 September 2026) groups failed checks by severity — "Needs attention" (warn/block) before "System notes" (info) — with the raw check name moved to a trailing tag instead of a colon prefix; `_decisions` now renders one `idea_card` per decision instead of a `white-space:nowrap` table row. **10 September 2026 — decisions before diagnostics:** `render_email`'s body order now puts both account sections directly after the banner (each `idea_card`'s left border colored to its action — green buy, red sell, amber trim, muted hold — and its badge carrying a directional glyph, ▲/▼/●) with `_health_line`, `other_sections`, and `_decisions` moved after a `_details_divider()` that says plainly nothing below it is hidden, only reordered |
...
| `test_emailer.py` | 98 tests, including escaping, no-research-on-abort, the `idea_card`/`idea_cards` bulleted format, the five-section cap (exact set, over-cap, unrecognised title, duplicate title, and that the cap is not enforced on an aborted run), closest-call reporting, `verify_email` (quantity mismatch, no matching decision, wrong-account decision, empty/unrecognised source, every allowed source prefix, untraceable and tolerance-matched numbers, date/ordinal exemption, and — 10 September 2026 — the required-quantity rule for `buy`/`sell`/`trim` parametrised against `hold`/`skip`/`none` needing none), that `render_email` cannot render an account section without `verify_email` passing first, (8 September 2026) `_health_line`'s severity grouping and `_decisions`'s card rendering, and (10 September 2026) `idea_card`'s action-colored border, `_action_badge`'s directional glyph, and `render_email`'s decisions-before-diagnostics section order |
