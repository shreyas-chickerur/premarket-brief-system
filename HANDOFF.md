...
| `test_ledger.py` | 143 tests: real broker order-history fixtures, split adjustment, the partially-filled-rest-cancelled fix, the opening-balance mechanism, the standing circuit-breaker fold, the fills/split-events cache, the sector and congress-discovery caches (same fold/horizon pattern), `all_traded_symbols` (including the fully-closed-position case), `days_since_last_brokerage_success`, monthly journal compaction, `run_entry` (schema pinning including `brokerage_ok`), `closed_for_scoring`, the state-bundle round trip (journal/fills/splits/sector/congress each individually, plus the central equivalence test: building a bundle from a real dated-file fold and unpacking it must reproduce that exact fold), `fills_cache_matches_fresh` (agreement, a split-adjusted mismatch flagged, out-of-overlap fills ignored, tolerance, no-order-id fills ignored), and `unpack_state_bundle`'s `extra_journal_files` (folds in after the bundle's own entries; defaults to none) |
...
#### Status snapshot (updated after each material change; latest 15 September 2026)

| # | Item | Status |
|---|---|---|
| 1 | Self-heal proves itself, including a real fix-and-retry cycle | **Done, for real, twice — and the authority behind it just got broader.** 9 September: diagnosed the account-mixing defect (`Fill` had no `account` field), wrote the fix, merged commit `85d0504`, verified live against real broker data (SGOV split 29.805637/4.421802 across the two accounts correctly). 14 September: diagnosed the Stage 0 bootstrap deadlock (`download_file_content`'s write-back cost scales with bytes of stored state, not file count), wrote a genuinely novel fix (`drive_snippets.py`, reading via `search_files` at max snippet verbosity instead), merged commit `9cc62d4`, verified live (695 tests, full reconciliation succeeded for the first time since 9 September). Both independently re-verified afterward (pulled the commit, ran the suite, confirmed none of the three absolute limits were touched). In between, two watchdog retries (10, 11 September) correctly diagnosed a capacity problem, found no safe fix available, and declined to guess rather than merge something risky — exactly the restraint the design calls for. 15 September: the watchdog's daily review pass (Stage 5B, `WATCHDOG_PROCEDURE.md`) went live and proved itself on its first real day, self-applying a genuine bug fix (`washsale.report_schema_problems`, commit `15be3fb`) without waiting for the next day's run. This item was the single biggest open question on this list and it is now closed. |
| 2 | Call quality over a stretch of days | **Still near zero — arguably the real gate now.** The two weeks since 5 September were dominated by infrastructure failures, not trading days: the account-mixing bug, four-plus consecutive Stage 0 timeouts, the bootstrap deadlock, and (14 September) a silent split-adjustment double-counting bug in the fills cache that corrupted cost basis/wash-sale data for six closed-out symbols for five days without tripping reconciliation. Stages 1 through 5 have barely run at all in that window — most days aborted before research started, and 14 September's own recovery consumed the whole pre-open window, so even a *successful* Stage 0 that day produced zero new research. 15 September surfaced yet another infrastructure gap in the same vein, on the write side this time: a full research day can burn the wall-clock budget before Stage 6 ever gets to write the fills-cache/state-bundle, which is now mitigated by writing a safety-net copy right after Stage 0 reconciliation (step 7b, `DAILY_PROCEDURE.md`) rather than waiting for Stage 6. There is still essentially nothing to evaluate "call quality" against yet. This is now the actual blocker, not a formality behind item 1. |
| 3 | Don't wait for full statistical power | **On track, unchanged.** Still `n=0`, correctly not gating on this. The OXY thesis opened 3 September matures **24 September 2026** — about a week and a half out — the first real test of the settlement/scoring path end to end. |
