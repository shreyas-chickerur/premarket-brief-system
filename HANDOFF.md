...
#### Status snapshot (updated after each material change; latest 14 September 2026)

| # | Item | Status |
|---|---|---|
| 1 | Self-heal proves itself, including a real fix-and-retry cycle | **Done, for real, twice.** 9 September: diagnosed the account-mixing defect (`Fill` had no `account` field), wrote the fix, merged commit `85d0504`, verified live against real broker data (SGOV split 29.805637/4.421802 across the two accounts correctly). 14 September: diagnosed the Stage 0 bootstrap deadlock (`download_file_content`'s write-back cost scales with bytes of stored state, not file count), wrote a genuinely novel fix (`drive_snippets.py`, reading via `search_files` at max snippet verbosity instead), merged commit `9cc62d4`, verified live (695 tests, full reconciliation succeeded for the first time since 9 September). Both independently re-verified afterward (pulled the commit, ran the suite, confirmed none of the three absolute limits were touched). In between, two watchdog retries (10, 11 September) correctly diagnosed a capacity problem, found no safe fix available, and declined to guess rather than merge something risky — exactly the restraint the design calls for. This item was the single biggest open question on this list and it is now closed. |
| 2 | Call quality over a stretch of days | **Still near zero — arguably the real gate now.** The two weeks since 5 September were dominated by infrastructure failures, not trading days: the account-mixing bug, four-plus consecutive Stage 0 timeouts, the bootstrap deadlock, and (14 September) a silent split-adjustment double-counting bug in the fills cache that corrupted cost basis/wash-sale data for six closed-out symbols for five days without tripping reconciliation. Stages 1 through 5 have barely run at all in that window — most days aborted before research started, and 14 September's own recovery consumed the whole pre-open window, so even a *successful* Stage 0 that day produced zero new research. There is essentially nothing to evaluate "call quality" against yet. This is now the actual blocker, not a formality behind item 1. |
