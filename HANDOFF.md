...
| File | What |
|---|---|
| `DAILY_PROCEDURE.md` | The canonical Stage 0–6 trading procedure, followed by both the main routine and the watchdog's retry — not code, but the single source of truth for what either one actually does. 18 September 2026: the state-bundle write/read is now chunked (see `ledger.py`'s row below), opening-balance reconciliation is now account-scoped, and Stage 6 ends with an unconditional, best-effort dead-man's-switch ping to a third, optional trigger-level placeholder, `{{HEALTHCHECKS_URL}}` — **not yet substituted in either trigger; see "Action needed" below** |
| `WATCHDOG_PROCEDURE.md` | The watchdog's own procedure, two jobs since 15 September 2026: (1) the original safety net — assess today's manifest, and on a real problem (`no_run`/`hung`/`aborted`), diagnose, attempt a fix, merge it, and re-run `DAILY_PROCEDURE.md` once, guaranteeing an email even on a broken day; (2) Stage 5B, a review pass that runs every day regardless of outcome — reads the day's own `metrics.optimizations` (`runlog.find_optimizations`'s already-computed output) and fixes what's safe to fix in place rather than letting a known issue sit. Infrastructure/config/doc findings fix on the same "concrete, narrow, well-understood" bar as an abort cause; strategy-adjacent findings (sizing, caps, thresholds) may also be self-applied, but only once the same finding has recurred across at least two separate days' manifests, never off a single day's observation. Both categories answer to the same three absolute limits as the original safety net. No second email — a Stage 5B fix takes effect starting the next run, recorded in the journal (`topic: "review_pass"`), not sent same-day |
| `research.py` | Deterministic Stage 1 research — replaces "research by web search" with a defined candidate universe and one parser per feed (news, congressional/insider activity, scheduled events, filings, macro, commodities, positioning), every item graded `ok`/`thin`/`degraded`/`failed` and required to attach to a symbol or channel. `candidates()` unions five sources as of 5 September 2026 (held, watchlist, top movers, `screener.py` results, politician-discovered symbols) — `SECTOR_MAP` is gone; `sector_from_company_overview`/`sector_from_etf_profile` and `congress_trade_items_by_politician`/`bioguide_ids_from_congress_items` replaced it. `NEWS_SENTIMENT` now filters on each article's own `ticker_sentiment` rather than trusting the requested symbol (the batching cross-wiring bug), and `macro_item`/`commodity_items` share the preview-envelope handling `CONGRESS_TRADES`/`INSIDER_TRANSACTIONS` already had. `universe_funnel` records how many symbols entered from each source. As of the SAME day, `candidates()`'s eligible universe (verified live against 229 real symbols the day this was built) is explicitly NOT what Stage 1 researches — `researched_set` bounds that to `DEFAULT_RESEARCH_SET_CEILING` (40), ranked by the gate's own first condition (`symbols_with_dated_catalyst`, reading the same bulk `EARNINGS_CALENDAR` pull for free) then congressional recency (`most_recent_congress_activity` on `recent_congress_items`' 90-day/$15,000-floor-bounded output) then liquidity, held positions always included regardless of the ceiling; `cut_for_budget` makes an over-tight ceiling visible instead of guessable |
| `screener.py` | The real Robinhood scanner as a candidate source (`get_scanner_filter_specs`, `create_scan`, `update_scan_filters`, `run_scan` — all previously uncalled). `CORE_UNIVERSE_FILTERS_V1` is enum-filter-only (`get_scanner_datapoints`/`preview_scan`, needed to validate an expression filter, were unavailable this session); `build_filters` adds a real `FILTER_TYPE_SECTOR` filter scoped to held positions' actual sectors; `affordable_for_agentic` narrows results to what the agentic account's current equity and weight cap could ever hold, computed fresh every call, never cached. `parse_scan_result` also keeps each result's `avg_volume` (5 September 2026) — the liquidity tiebreak `research.researched_set` ranks by, from data the scan already returned. The real scan's identifier is deliberately not in this file or anywhere else in the public repo — see `HANDOFF.private.md` |
...
| `runlog.py` | Run manifests, staged timing (against per-stage budgets, `STAGE_TIMING_BUDGETS_MS`/`stage_budget_overruns` — `preflight`'s budget corrected from an unmeasured 15s to 180s, 5 September 2026; `gather`'s corrected the same day from an equally unmeasured 120s to 1,800s once `research.researched_set` bounded what that stage actually costs), preflight self-audit (including a blocking `journal_fully_readable` check and, new the same day, `washsale_registry_stable`, block-severity), verified market calendar, regression review, optimization proposals (`stop_distance` removed — see section 11), real `stop_filled` decisions via `stop_filled_decision`, the pinned `GATE_CONDITIONS` five-condition order and `closest_calls` gate-miss ranking, `gate_funnel` (reached/cleared counts, distinct from a housekeeping decision that never touched the gate), `brokerage_token_health` (forecasts the observed token-expiry window before a run aborts on it), `stage0_capacity_forecast`/`stage0_read_cost_metric` (10 September 2026 — the same early-warning pattern as the token forecast, applied to Stage 0's own growing Drive-read cost: self-calibrates from each run's own observed seconds-per-file, aborted runs included, and reports how many trading days of headroom remain before the wall-clock deadline is projected to be exceeded), honest scoring. Per-stage budgets are advisory only — nothing stopped a stage that ran long — so `elapsed_seconds`/`state.json.config.run_wall_clock_deadline_seconds` (4200 seconds as of 10 September 2026, previously 2700 — live-only, not a code constant; `DEFAULT_WALL_CLOCK_DEADLINE_SECONDS` is just the fallback) give `DAILY_PROCEDURE.md` a real stop between stages, and `heartbeat_payload` is the run's own liveness signal for `watchdog.py` to read from outside, for the one failure mode a self-check can never catch: a single hung call the deadline check itself never gets to run against |
...
| `ledger.py` | Positions and the wash-sale trade list rebuilt from broker order history; `all_traded_symbols(fills)` (the split-check universe, derived from fills, not from held positions, which had silently excluded every closed-out symbol); the append-only journal fold, `Journal.days_since_last_brokerage_success` and `latest_washsale_report`; the bounded-staleness fills/split-events cache plus the newer sector cache (180-day horizon) and congress-discovery cache (weekly horizon), all the same create-only dated-file pattern; optional monthly journal compaction, exactly equivalent to the daily fold it replaces; `run_entry(log)` pins the `"run"` journal entry's schema to exactly what `find_optimizations` reads, now including `brokerage_ok`; `Journal.closed_for_scoring` joins thesis + outcome entries for `score_closed_decisions`; `build_state_bundle`/`unpack_state_bundle` (11 September 2026) collapse Stage 0's ~20-file read into one `state-bundle-*.json`, by round-tripping the bundle's stored rows back through the SAME `fold_journal`/`fold_fills_cache`/`fold_splits_cache`/`fold_sector_cache`/`fold_congress_discovery_cache` functions the dated files use, so a bundle and the files it summarises are guaranteed to fold identically — one parser, not two; `fills_cache_matches_fresh` (14 September 2026) cross-checks cached fills against a fresh re-fetch of the same order_ids, catching a cache that holds `apply_splits`'s OUTPUT instead of its input — `apply_splits` is deliberately not idempotent, so caching its result compounds every run, silently, for any symbol already closed out (a net-zero position reconciles clean regardless); `unpack_state_bundle`'s `extra_journal_files` parameter (15 September 2026) folds in any real `journal-*.json` files dated on/after the bundle's own `as_of` on top of the bundle's stored entries — the read-side counterpart of the new Stage 0 step 7b early write (`DAILY_PROCEDURE.md`), which writes a safety-net fills-cache/splits-cache/state-bundle immediately after reconciliation succeeds rather than waiting for Stage 6, so a run that never reaches or finishes Stage 6 still leaves tomorrow something better than the full per-file bootstrap to read; `state_bundle_chunks`/`group_state_bundle_titles`/`merge_state_bundle_chunks` (18 September 2026) chunk the bundle the same way `fills_cache_chunks` chunks the fills cache — a real bundle (361,643 bytes) has never fit `create_file`'s inline-only content limit — but a bundle is a full-state SNAPSHOT, not an append-only accumulation, so the filename now carries `run_seq`/`chunk_seq`/`chunk_count` explicitly and `group_state_bundle_titles` verifies a group is COMPLETE from the Drive listing alone before ever downloading it, so an aborted partial write is never mistaken for the whole state; `Journal.opening_balances_for_account(account)` (18 September 2026) is the account-scoped counterpart of the pre-existing `opening_balances` — that unscoped property merges every recorded entry across both accounts, and passing it into the WRONG account's `reconcile_positions` invents a phantom residual (found while re-reading Stage 0 step 7 to design the chunking fix above; not yet a real incident, since the 18 September watchdog run had already scoped it correctly by inference) |
...
| `emailer.py` | HTML brief rendering, failure diagnosis (`diagnose()` tells a lapsed brokerage token apart from a routine missing its connectors entirely), subject lines, `verify_email` — raises on an unsupported number, source, empty-source bullet, or a `buy`/`sell`/`trim` card with no parseable quantity (`ACTIONS_REQUIRING_QUANTITY`, 10 September 2026 — "partial trim" is not an answer to "how much"); `render_email` takes structured account ideas (not pre-rendered HTML) and runs `verify_email` on them itself, unconditionally. Each `idea_card`'s left border is colored to its action (green buy, red sell, amber trim, muted hold) and its badge carries a directional glyph (▲/▼/●). **10 September 2026, twice in one day, both at the reader's own request.** First: decisions before diagnostics — both account sections render directly after the banner. Second, more drastic: the email was cut from five `CANONICAL_SECTIONS` to three — "Agentic account — activity," "Individual account — suggestions," "System health" — dropping "Prior-day review" and "Diversification" (still computed and journaled every run, just no longer emailed) and the entire per-check `_health_line` dump (`_decisions()` and the "Needs attention"/"System notes" grouping both deleted outright) in favor of one code-computed tally sentence (duration, pass count) plus a short, caller-written, genuinely plain-English "System health" summary — a raw check's `detail` text is written for a developer debugging that run, not for someone deciding whether to trust today's ideas, and no amount of automatic reformatting bridges that gap. `OTHER_SECTIONS` is now `("System health",)`; the now-unreachable duplicate-title check was deleted rather than left dead. **The "Not investment advice..." disclaimer is now gone from `_footer` entirely (11 September 2026)** — a `disclaimer: bool = True` parameter meant it printed on every path including every abort, since `DAILY_PROCEDURE.md`'s one call site never passed it, despite two prior requests to remove it. There is no flag left to regress: the line and the parameter are both deleted, not re-defaulted |
| `watchdog.py` | The outside check: did the daily run happen at all, and was it healthy — catches a hung run that never reached its own email. As of 5 September 2026, `latest_heartbeat_for`/`heartbeat_status` read `run-heartbeat-*.json` to tell a run that is merely slow (`kind="alive"`, not a problem) apart from one genuinely stuck (`kind="hung"`, stale past `HEARTBEAT_STALE_AFTER_SECONDS`) — previously both looked identical to `no_run` |
...
| `test_runlog.py` | 113 tests, including daylight-saving drift, market-calendar integrity, circuit-breaker enforcement, the blocking `journal_fully_readable` check, `abort()`'s first-reason-wins semantics, `stop_filled_decision` against real order shapes, `closest_calls` ranking, `stage_budget_overruns`, `gate_funnel`, `brokerage_token_health`, `stage0_capacity_forecast`/`stage0_read_cost_metric` (comfortable margin, inside the warn window, already over budget, and that a stale early observation never masks a worse recent one), `washsale_registry_stable` (the regression case, legitimate-expiry exemption, proxy-warning exemption), `elapsed_seconds`/`last_stage_name`, and `heartbeat_payload` (including its round-trip directly into `watchdog.heartbeat_status`) |
| `test_washsale.py` | 34 tests, including `Registry.report`'s exact-match to `blocked_symbols` on the regression scenario |
| `test_ledger.py` | 169 tests: real broker order-history fixtures, split adjustment, the partially-filled-rest-cancelled fix, the opening-balance mechanism (including the 18 September account-scoped `opening_balances_for_account`), the standing circuit-breaker fold, the fills/split-events cache (including `fills_cache_chunks`), the sector and congress-discovery caches (same fold/horizon pattern), `all_traded_symbols` (including the fully-closed-position case), `days_since_last_brokerage_success`, monthly journal compaction, `run_entry` (schema pinning including `brokerage_ok`), `closed_for_scoring`, the state-bundle round trip (journal/fills/splits/sector/congress each individually, the central equivalence test against a real dated-file fold, `extra_journal_files`, and the 18 September chunking/grouping/merging trio — reassembly matches the unchunked bundle, an incomplete group is never mistaken for a complete one, a mismatched `as_of` across chunks is reported not merged), and `fills_cache_matches_fresh` (agreement, a split-adjusted mismatch flagged, out-of-overlap fills ignored, tolerance, no-order-id fills ignored) |
| `test_evidence.py` | 28 tests, including known-answer edge detection and futility |
| `test_watchdog.py` | 36 tests, including `latest_heartbeat_for` (the `-N` sequence-preference rule, mirroring `latest_manifest_for` exactly), `heartbeat_status` (alive/hung/absent, boundary-exact, malformed-heartbeat-is-hung), and `assess` with a heartbeat (the alive-not-no_run and hung-not-no_run cases, and a completed manifest always winning over a stale heartbeat) |
| `test_research.py` | 131 tests, against recorded fixtures in `fixtures/research/`, never the live API — including the `NEWS_SENTIMENT` cross-wiring regression, the macro/commodity preview branches, `sector_from_company_overview`/`sector_from_etf_profile`, `congress_trade_items_by_politician`, `universe_funnel`, `researched_set` (tier priority, the held-always-included rule, `cut_for_budget`), `symbols_with_dated_catalyst`, and `recent_congress_items`' recency/materiality bound |
| `test_screener.py` | 21 tests, against `fixtures/scanner/`, the real recorded `get_scanner_filter_specs`/`run_scan` responses -- not hand-built, and not matching either tool's own docstring shape |
| `test_procedure_docs.py` | 6 tests: the DRY RUN guard's exact text, and that every stage has a matching rationale section |
| `test_emailer.py` | 92 tests, including escaping, no-research-on-abort, the `idea_card`/`idea_cards` bulleted format, `idea_card`'s action-colored border and `_action_badge`'s directional glyph, the three-section cap (exact set, over-cap, unrecognised title, and that the cap is not enforced on an aborted run — the old duplicate-title case was deleted 10 September 2026 along with the code path it tested, made unreachable once the cap dropped to 1), closest-call reporting, `verify_email` (quantity mismatch, no matching decision, wrong-account decision, empty/unrecognised source, every allowed source prefix, untraceable and tolerance-matched numbers, date/ordinal exemption, and the required-quantity rule for `buy`/`sell`/`trim` parametrised against `hold`/`skip`/`none` needing none), that `render_email` cannot render an account section without `verify_email` passing first, that both account sections render before "System health," that the health line is one plain sentence (duration, pass count) with no per-check dump, no snake_case identifiers, and a caller's plain-English summary still rendering underneath it, and that the disclaimer is gone on every path including an aborted one (11 September 2026, replacing the old present-by-default test) |
...
| # | Item | Status |
|---|---|---|
| 1 | Self-heal proves itself, including a real fix-and-retry cycle | **Done, for real, three times now.** 9 September: diagnosed the account-mixing defect (`Fill` had no `account` field), wrote the fix, merged commit `85d0504`, verified live against real broker data (SGOV split 29.805637/4.421802 across the two accounts correctly). 14 September: diagnosed the Stage 0 bootstrap deadlock (`download_file_content`'s write-back cost scales with bytes of stored state, not file count), wrote a genuinely novel fix (`drive_snippets.py`), merged commit `9cc62d4`, verified live (695 tests, full reconciliation succeeded for the first time since 9 September). 15 September: the watchdog's daily review pass (Stage 5B) went live and proved itself the same day, self-applying `washsale.report_schema_problems` (commit `15be3fb`). 18 September: after a genuine 06:20 abort (`drive_snippets.unescape_snippet` mishandled a doubled backslash), the watchdog diagnosed it, fixed it (`14e07d9`), and re-ran the full procedure end to end successfully — 885 broker orders, 875 fills, both accounts in exact agreement — then its own Stage 5B pass caught and fixed a second, independent bug the same day (a wash-sale report account-mislabeling gap in `DAILY_PROCEDURE.md` step 8, commit `288477a`) and chunked the fills-cache write past a transport wall that had silently failed three days running (commit `cca2261`, first successful write since 8 September). This item stays closed; every new incident so far has been diagnosed and fixed correctly, never guessed past. |
| 2 | Call quality over a stretch of days | **Still near zero — the real gate.** Infrastructure firefighting has now run five separate incidents (account-mixing, bootstrap deadlock, split-adjustment double-counting, the write-side budget wall, the backslash-escaping defect) across two weeks, each fixed in turn, but each also ate the day it was found on. 17 September added a new failure shape entirely: a shared weekly Claude usage cap silently killed BOTH the 06:20 routine and the 08:20 watchdog within seconds of starting — no heartbeat, no manifest, nothing — invisible from Drive alone. Mitigated by `deadman_switch.gs` (see "Action needed" below for the one step still pending). There is still essentially nothing to evaluate "call quality" against. This is the actual blocker, not a formality behind item 1. |
| 3 | Don't wait for full statistical power | **On track, unchanged.** Still `n=0`, correctly not gating on this. The OXY thesis opened 3 September matures **24 September 2026** — the first real test of the settlement/scoring path end to end. |
...
**Action needed (one step only you can do, ~3 minutes): install the dead-man's switch.** It is `deadman_switch.gs`, a Google Apps Script that runs in your own Google account (no third-party service, nothing depending on Claude credits) and emails you if no brief has arrived by 09:15 Central on a weekday. (It replaces the 18 September `{{HEALTHCHECKS_URL}}` ping, which needed an account and outbound network access from the routine that could not be verified.) Steps: script.google.com > New project > paste the file over `Code.gs` > select `setup` > Run > approve Gmail access > optionally run `testAlert` to see the email. `setup` creates the daily trigger. The check logic is tested (`test_deadman_switch.py`); the Gmail glue can only run in your account.

Net: the hard part everyone worried about (can the self-heal loop be trusted) turned out well — it has now diagnosed and correctly fixed two real, non-trivial defects, and correctly declined to guess twice more. The remaining blocker is mundane by comparison but just as real: the system needs a stretch of **boring** days — Stage 0 finishing quickly, real research happening, real ideas (accepted or rejected) showing up in the email — before item 2's clock can even start. It hasn't had one yet. **Next scheduled reassessment: 24 September 2026** (OXY thesis maturity) or sooner if a real stretch of quiet, working days accumulates first.
...
`PBS Core Universe v1` — its scan_id is not recorded here, see
`HANDOFF.private.md`, same as an account number —
...
- **`runlog.STAGE_TIMING_BUDGETS_MS`'s values are still judgment calls for
  most stages, not measurements**, the same category as
  `quantcore.gap_risk_haircut` — this system does not yet have enough real
  per-stage timing history to calibrate all of them against. `"preflight"`
  (180s) and `"gather"` (1,800s, 5 September 2026) are the two exceptions —
  each is pinned to real observed durations (see PROCEDURE_RATIONALE.md for
  both) — but the rest have never been checked against a real run's actual
...
- **The brokerage token: mostly answered by the journal (21 September 2026).**
  The old note said it expires roughly every four days; that was one early
  observation. Since 8 September the broker was read successfully on 8, 9, 11,
  14, 15, 16, 18 and 21 September -- thirteen days on one token, surviving a
  3-day idle gap every weekend -- so it is refreshed by use. Still unknown: how
  long an IDLE gap it survives beyond 3 days, and whether it ever expires
  outright. `runlog.brokerage_token_health` now warns only past 3 idle days, and
  `brokerage_ok` is derived from the `tools_available` check so the forecast
  no longer counts days since a run merely forgot to log a call. If the token
  does lapse, runs abort at the tool-visibility check until re-authorised.
...
- **A scheduled routine can stall indefinitely on a sandbox permission
  ...
  oversized `get_equity_orders` page), not observed since. As of 5 September
  2026, this is partially addressed, not eliminated: `runlog.heartbeat_payload`
  + `watchdog.heartbeat_status` now DETECT a stall from outside — a run that
  goes quiet past `watchdog.HEARTBEAT_STALE_AFTER_SECONDS` (35 minutes) is
  `kind="hung"`, a specific, actionable finding distinct from `no_run`, and
  the watchdog's retry no longer has to guess whether a still-quiet run is
  slow or actually stuck. What this does NOT do: recover the stuck session
  itself (there is still no way to interrupt a hung external call from
  outside it — the retry is a fresh run, not a resumption) or guarantee the
...
- **The screener (`screener.py`) has never run inside a real
  `DAILY_PROCEDURE.md` execution.** `PBS Core Universe v1` (scan_id in
  `HANDOFF.private.md`) was created and exercised manually in the session
  that built it — real filters, real 397-instrument results, real fixture
  — but no actual daily run has ever called
...
- **`research.researched_set` has been exercised against real, live data
  once (5 September 2026) but never inside a real `DAILY_PROCEDURE.md`
  execution.** A live rehearsal built the real eligible universe (229
  symbols: 23 held, 200 from the real scanner, 8 from real congressional
  discovery after the recency/materiality bound) and ran it through
  `researched_set` with a real `EARNINGS_CALENDAR` pull, real congressional
  recency data, and real screener liquidity — 40 researched, 189 cut for
  budget, tiers populated from all three non-held sources. `cut_for_budget`
  at 189 against 229 eligible is a real first data point on whether 40 is
  the right ceiling, not yet a trend — it needs several real daily runs
  before concluding anything about whether it should move. `gather`'s
  re-timed 1,800s budget rests on the same rehearsal's per-call timing plus
  the pre-ceiling ~25s/symbol figure, not yet on a full real run at the new
  ceiling end to end.
