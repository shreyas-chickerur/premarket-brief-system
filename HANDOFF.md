...
| `test_runlog.py` | 107 tests, including daylight-saving drift, market-calendar integrity, circuit-breaker enforcement, the blocking `journal_fully_readable` check, `abort()`'s first-reason-wins semantics, `stop_filled_decision` against real order shapes, `closest_calls` ranking, `stage_budget_overruns`, `gate_funnel`, `brokerage_token_health`, `washsale_registry_stable` (the regression case, legitimate-expiry exemption, proxy-warning exemption), `elapsed_seconds`/`last_stage_name`, and `heartbeat_payload` (including its round-trip directly into `watchdog.heartbeat_status`) |
