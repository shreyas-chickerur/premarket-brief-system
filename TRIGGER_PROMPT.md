...
> 
> Derive positions with `ledger.positions_from_fills` and compare against `get_equity_positions` with `ledger.reconcile_positions`. **Any remaining disagreement aborts execution and is reported** — once splits are accounted for, the broker's own positions failing to follow from the broker's own fills means a transfer, a different corporate action, or a bug, and every downstream number depends on knowing which.
